...
void PLAYERWAVE_iniciaMusica(unsigned char indice,unsigned char modo){
  ...
  if(indice==1)
    volume = PARAMETROS_leVolumeSom();
  else
    volume = PARAMETROS_leVolumeVoz();
...
void PLAYRWAVE_rti(void){
  ...
  temp = MEMORYWRAPPER_readSingle(enderecoInicialMusica++)<<2;
  if(temp){
    temp*= volume;
    temp/= 100;
  }
  
  DACR_bit.VALUE = temp;//MEMORYWRAPPER_readSingle(enderecoInicialMusica++)<<1;//*volume)/100;
  if(tamanhoMusica){
    tamanhoMusica--;
    T0MCR_bit.MR0I =
      1;    
