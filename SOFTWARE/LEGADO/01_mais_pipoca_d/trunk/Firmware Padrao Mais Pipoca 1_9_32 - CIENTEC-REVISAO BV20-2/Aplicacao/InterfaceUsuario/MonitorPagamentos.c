...
void MP_verificaFilaPagamentos(void){
  ...
  static unsigned short int last_evento;
  char linha[17];
    ...
    if(CCTALK_leFilaEventos(END_NOTEIRO,&quantidade,eventos)){
      ...
      while(quantidade!=quantidadeAnterior){
        ...
        #ifdef _DEBUG_MENSAGEMS_
        if((eventos[0]>>8) != BILL_VALIDATED_OK){
          sprintf(linha,"Mensagens:%d\n\r",quantidade);
          DEBUG_PRINT(linha);
          DEBUG_PRINT("1:");
          DEBUG_PRINT(CCTAK_msg_to_string(eventos[0]>>8));
          DEBUG_PRINT("2:");
          DEBUG_PRINT(CCTAK_msg_to_string(eventos[1]>>8));
          DEBUG_PRINT("3:");
          DEBUG_PRINT(CCTAK_msg_to_string(eventos[2]>>8));
          DEBUG_PRINT("4:");
          DEBUG_PRINT(CCTAK_msg_to_string(eventos[3]>>8));
          DEBUG_PRINT("5:");
          DEBUG_PRINT(CCTAK_msg_to_string(eventos[4]>>8));
        }
        #endif
