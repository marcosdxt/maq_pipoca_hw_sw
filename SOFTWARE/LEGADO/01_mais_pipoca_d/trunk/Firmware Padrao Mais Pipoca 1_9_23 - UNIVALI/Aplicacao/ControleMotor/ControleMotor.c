...
int MU_calcula_integral_erro(int erro,unsigned flush){
  ...
  if(flush){
    ...
    y = sadd(y,acumulado); // Soma com saturação, 32 bits com sinal           
    acumulado = y;
...
void MU_controleVelocidade(void){
  ...
  const unsigned int kd_const = KD*(1<<15);
  const unsigned int ki_const = KI*(1<<15);
  ...
  if(CM_ligaMotor){   
    ...
    referencia_rampa = 0;//ATRASO_MAXIMO;
    ciclos = 30;//90;
  }  
}
/***********************************************************************************
