...
|                                          no loop de falha da resist�ncia e no
|                                          de sinaliza��o de falha
|                             (20/05/2017) Alterado a forma como faz a limpeza da
|                                          m�quina durante o preparo. Agora utiliza
|                                          a fun��o serve_pipoca.
|                                          Dentro do menu pipoca_teste havia uma 
|                                          chamada errada para a limpeza, onde a velocidade
|                                          estava com o par�metro do valor da pipoca
|                                          Criado menu para reiniciar a senha mestre,
|                                          para utiliz�-lo � necess�rio usar o gerador
|                                          de contra-senha
| __________________________________________________________________________________
*/
...
/***********************************************************************************
*       Descri��o       :       Fun��o principal da interface com o usu�rio
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_entry(void*pPar){
  ...
  CA_ini();// Controle de amostra gr�tis
  ...
  // Faz a verifica��o inicial da resist�ncia
  // e trava o processo se n�o localizar    
  HD44780_clearText();
  ...
  for(;;){
    ...
    if(!codigo){
      ...
      HD44780_posicionaTexto(7,1);  // Remove as indica��es de erro do 
      ...
      ///---------------------------------------------------///
      ///                                                   ///
      /// Faz o modo de funcionamento de locaca��o          ///
      ///                                                   ///
      ///---------------------------------------------------///
      static unsigned char flagLocacao=0;      
      if(PARAMETROS_leFlagLocacao()){
        IU_propaganda();  // Faz a propaganda              
        flagLocacao = 1; // Esse flag fica setado para indicar que precisar reiniciar o noteiro quando saida da loca��o
      ...
      }//Fim do inicio da prepara��o
      
      IU_creditoPipoca = 0;          
   }//Fim da verifica��o c�clica
   else{          
     // Falha no teste de verifica��o 
     // funcional     
     for(;;){
       TELET_enviaEstado(ESTADO_MONITORACAO);
...
/***********************************************************************************
*       Descri��o       :       Desenha os erros nos displays da m�quina
*       Parametros      :       (unsigned char) c�digo
*       Retorno         :       nenhum
***********************************************************************************/
void IU_escreveErros(unsigned char codigo){
  ...
  switch(codigo){
      ...
      case 3: HD44780_writeString("| E03");
              ...
              bloqueado = 1;
              break;        
      // N�o foi detectada rota��o do ventilador
...
/***********************************************************************************
*       Descri��o       :       Escreve a mensagem iniciando preparo
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_mensagemIniciandoPreparo(void){
...
/***********************************************************************************
*       Descri��o       :       Escreve a temperatura de preparo no lcd interno
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_escreveTemperaturaResistencia(unsigned char rpm){
...
/***********************************************************************************
*       Descri��o       :       Getter para o IU_falhaDosador
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
unsigned char IU_getFalhaDosador(void){
...
/***********************************************************************************
*       Descri��o       :       Getter para o IU_falhaMotor
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
unsigned char IU_getFalhaMotor(void){
...
/***********************************************************************************
*       Descri��o       :       Escreve o valor da pipoca no display de cliente
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_desenhaPrecoPipoca(void){
...
/***********************************************************************************
*       Descri��o       :       Escreve no display o total de dinheiro inserido
*                               pelo usu�rio
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
...
/***********************************************************************************
*       Descri��o       :       Inicia o procedimento de prepara��o da pipoca
*       Parametros      :       nenhum
*       Retorno         :       (unsigned char) c�digo de erro
***********************************************************************************/
unsigned char IU_preparaPipoca(void){
  ...
  // Recodifica os erros para o padr�o da
  // fun��o principal, corrigir isso algum dia!
  switch(res){
    case 0 : break;
    case 1 : return 5;
    case 2 : return 4;   
  ...
  // Seta a temperatura de processo na resist�ncia
  MU_setTemperatura(tempResf,fator);  
  MU_setRPM(v1);  
  
  // Aguarda a resist�ncia atingir a temperatura de processo
  ...
  */
  ...
  // antes desligando o motor e a resist�ncia
  if(!DM_aplicaDosagem()){
     MU_setRPM(0); 
  ...
  for(unsigned char i=0;i<tempo;i++){
    ...
    // Motor sem rota��o � falha
    // no fus�vel ou sensor
    if(!MU_getRPMmedido()){
       MU_setRPM(0); 
...
/***********************************************************************************
*       Descri��o       :       Fica rolando as mensagens no display durante
*                               a fase onde aguarda a resist�ncia atingir
...
/***********************************************************************************
*       Descri��o       :       Limpeza da pipoqueira com ar
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_limpezaPipoqueira(unsigned short int velocidade){
...
/***********************************************************************************
*       Descri��o       :       Piscada da fita de leds e exibi��o da mensagem
*                               de propaganda
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
...
/***********************************************************************************
*       Descri��o       :       Exibe o contador de vendas no display interno
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_exibeContadorVendas(void){       
...
/***********************************************************************************
*       Descri��o       :       Tela que exibe no display externo
...
/***********************************************************************************
*       Descri��o       :       Atualiza os totalizadores 
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_atualizaTotalizadores(unsigned int valorPipoca){
  ...
  // Totalizador de hist�rico de faturamento
...
/***********************************************************************************
*       Descri��o       :       Verifica��o ciclica
*       Parametros      :       nenhum
*       Retorno         :       (unsigned char) maior do que zero
*                                               sen�o houverem falhas
*                                               no hardware
***********************************************************************************/
unsigned char IU_verificacaoCiclica(void){
...
/***********************************************************************************
*       Descri��o       :       Tela com o pr�-aquecimento da resist�ncia
*       Parametros      :       nenhum
*       Retorno         :       (unsigned char) maior do que zero
...
/***********************************************************************************
*       Descri��o       :       Interface para servir a pipoca ao usu�rio
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_servePipoca(unsigned int inicial,unsigned int final){
...
/***********************************************************************************
*       Descri��o       :       Faz a rolagem da mensagem na segunda linha
...
/***********************************************************************************
*       Descri��o       :       Oscila a fita de leds durante a prepara��o
*       Parametros      :       nenhum
*       Retorno         :       nwenhum
***********************************************************************************/
void IU_oscilaFitaPreparacao(void){
...
/***********************************************************************************
*       Descri��o       :       Envia os dados de telemetria para o m�dulo
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_enviaTelemetria(void){
...
/***********************************************************************************
*       Descri��o       :       Stamp do estado da conex�o com 
*                               o m�dulo de telemetria
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_stampEstadoConexaoModuloTelemetria(void){
...
/***********************************************************************************
*       Descri��o       :       Interface para resfriamento da
*                               resist�ncia
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
...
/***********************************************************************************
*       Descri��o       :       Fun��o que faz o aquecimento da panela 
...
/***********************************************************************************
*       Descri��o       :       Faz a verifica��o inicial da resist�ncia
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
unsigned char IU_verificacaoInicialResistencia(void){
...
/***********************************************************************************
*       Descri��o       :       Timer hook para o compensador
...
/***********************************************************************************
*       Descri��o       :       Chamada para o compensador que ocorre
*                               a cada prepara��o de pipoca
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_cicloCompensador(void){
...
/***********************************************************************************
*       Descri��o       :       Getter para o n�mero de s�rie da m�quina
*       Parametros      :       nenhum
*       Retorno         :       (unsigned int) n�mero de s�rie da m�quina
***********************************************************************************/
...
/***********************************************************************************
*       Descri��o       :       Exibe o n�mero de s�rie da placa
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
void IU_exibeNumeroSerie(void){
