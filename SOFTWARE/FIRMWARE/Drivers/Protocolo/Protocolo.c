      ...
      if(PROTOCOLO_checksum(PROTOCOLO_bufferRx,PROTOCOLO_bufferRx[1]-1)==PROTOCOLO_bufferRx[PROTOCOLO_bytesRecebidos-1]){      
        switch(PROTOCOLO_bufferRx[0]){
         ...
         case WRITE_PARAMETER:
              PROTOCOLO_decodeEscreveParametro(PROTOCOLO_bufferRx[2],PROTOCOLO_bufferRx[3]<<8 | PROTOCOLO_bufferRx[4]);
              break;
         case READ_PARAMETER:
              PROTOCOLO_decodeLeParametro(PROTOCOLO_bufferRx[2]);
              break;
         case WRITE_CLIENT_MESSAGE:              
              PROTOCOLO_decodeMensagemUsuario(PROTOCOLO_bufferRx[2],&PROTOCOLO_bufferRx[3]);
              break;
         case ASSINA_PLACA:
...
void PROTOCOLO_decodeEscreveTabelaMusicas(unsigned char musica,
  ...
  if(musica>3){
    ...
    PARAMETROS_alteraTabelaIndiceMusicas(musica,enderecoInicial + AREA_AUDIO,tamanho);
...
void PROTOCOLO_decodeLeituraTabelaMusicas(unsigned char musica){
  ...
  if(musica>3){
    ...
    PARAMETROS_carregaDadosIndiceMusicas(musica,&enderecoInicial,&tamanho);
...
/***********************************************************************************
*       Descrição       :       Escreve em um dos parametros do sistema
...
void PROTOCOLO_decodeEscreveParametro(unsigned char indice,unsigned short int valor){
  ...
  PARAMETROS_escreveParametro(indice,valor);
  
  PROTOCOLO_bufferTmp[0] = WRITE_PARAMETER;
  PROTOCOLO_bufferTmp[1] = 6;
  PROTOCOLO_bufferTmp[2] = indice;
  PROTOCOLO_bufferTmp[3] = valor>>8;
  PROTOCOLO_bufferTmp[4] = valor;
  PROTOCOLO_bufferTmp[5] = PROTOCOLO_checksum(PROTOCOLO_bufferTmp,5);
  
  PROTOCOLO_enviaPacote(PROTOCOLO_bufferTmp,6);  
}
/***********************************************************************************
*       Descrição       :       Lê um dos parametros do sistema
*       Parametros      :       (unsigned char) indice do parametro
*       Retorno         :       nenhum
***********************************************************************************/
void PROTOCOLO_decodeLeParametro(unsigned char indice){
  unsigned short int parametro = PARAMETROS_leParametro(indice);
  
  PROTOCOLO_bufferTmp[0] = READ_PARAMETER;
  PROTOCOLO_bufferTmp[1] = 6;
  PROTOCOLO_bufferTmp[2] = indice;
  PROTOCOLO_bufferTmp[3] = parametro>>8;
  PROTOCOLO_bufferTmp[4] = parametro;
  PROTOCOLO_bufferTmp[5] = PROTOCOLO_checksum(PROTOCOLO_bufferTmp,5);
...
/***********************************************************************************
*       Descrição       :       Decodifica o comando de escrita em uma das
*                               mensagens de cliente
*       Parametros      :       (unsigned char) indice
*                               (unsigned char*) ponteiro para a mensagem
*       Retorno         :       nenhum
***********************************************************************************/
void PROTOCOLO_decodeMensagemUsuario(unsigned char indice,unsigned char *msg){
  
  if(indice<4){
    PARAMETROS_leMensagemCliente(indice,(char*)msg);
 
    PROTOCOLO_bufferTmp[0] = READ_PARAMETER;
    PROTOCOLO_bufferTmp[1] = 6;
    PROTOCOLO_bufferTmp[2] = indice;    
    PROTOCOLO_bufferTmp[3] = PROTOCOLO_checksum(PROTOCOLO_bufferTmp,5);
    PROTOCOLO_enviaPacote(PROTOCOLO_bufferTmp,4);      
  }
  else{
...
/***********************************************************************************
*       Descrição       :       Faz a assinatura da placa
*       Parametros      :       nenhum
*       Retorno         :       nenhum
***********************************************************************************/
...
void PROTOCOLO_decodeAssinaPlaca(void){
  char bufferAssinatura[64];
  
  for(unsigned char i=0;i<64;i++)
    bufferAssinatura[i] = (char)PROTOCOLO_bufferRx[2+i];
  ...
  if(ASSINATURA_decodifica(bufferAssinatura,&senha,&endereco,&presetValores,&contadorVendasParcial,&contadorVendasTotal,&arrecadacaoParcial,&arrecadacaoTotal)){
    
    if(senha==ASSINATURA_contraSenhaLocal){
      
      ASSINATURA_gravaAssinatura(endereco,presetValores,contadorVendasParcial,contadorVendasTotal,arrecadacaoParcial,arrecadacaoTotal);                 
      res = ASSINADO_COM_SUCESSO;
    }
    else
      res = ASSINATURA_INVALIDA;      
  }
  else
    res = ASSINATURA_INVALIDA;
      
  PROTOCOLO_bufferTmp[0] = ASSINA_PLACA;
  PROTOCOLO_bufferTmp[1] = 4;
  PROTOCOLO_bufferTmp[2] = res;
  PROTOCOLO_bufferTmp[3] = PROTOCOLO_checksum(PROTOCOLO_bufferTmp,3);
  PROTOCOLO_enviaPacote(PROTOCOLO_bufferTmp,4);
}
/***********************************************************************************
